...
async def brand_presence_scan(brand: str, domain: str) -> dict:
    ...
    results = await tf.tf_search_many(queries, max_results=3, prefer="tinyfish_only")
    prepared = []
    candidate_urls = set()
...
async def pr_list_scan(brand: str, domain: str) -> list:
    ...
    for q in queries:
        for pg in (1, 2):
            tasks.append(tf.tf_search(q, max_results=12, page=pg, prefer="tinyfish_only"))
...
async def discover_reviews(brand: str, domain: str) -> dict:
    ...
    for pi, (label, host) in enumerate(plats):
        if host == "google.com":
            ...
            qmap.append(pi)
    all_results = await tf.tf_search_many(queries, max_results=3, prefer="tinyfish_only")
